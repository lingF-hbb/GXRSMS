...
<div class="layui-fluid">
    <div class="layui-card">
        ...
        <div class="layui-card-body">
            ...
            <table id="goodInfoQuery" lay-filter="LAY-app-content-comm"></table>
...
<script>
    layui.config({
        ...
        table.render({
            elem: '#goodInfoQuery'
            , url: '${request}/good/queryAllgood.html' //向后端默认传page和limit
            , cols: [[
                ...
                , {field: 'detail', title: '奖惩明细', sort: true}/*改为字段*/
                , {field: 'reason', title: '奖惩原因', sort: true}/*改为字段*/
        ...
        $("#query-all-info").click(function () {
            table.reload('goodInfoTable', {
                url: '${request}/good/queryAllgood.html'
                , request: {
                    pageName: 'pageNum',
                    limitName: 'pageSize'  //如不配置，默认为page=1&limit=10
                }
        ...
        $("#query").click(function () {
            var goodInfo = $("#searchInfo").val();
            if(goodInfo==""||goodInfo==null){
                layer.msg('请输入查询内容(员工姓名/编号/部门名称~)', {
                    offset: '15px'
                    , icon: 2
                    , time: 1000
                });
                ...
                //执行重载
                table.reload('goodInfoTable', {
                    url: '${request}/good/querygoodInfo.html' //向后端默认传page和limit
                    , where: { //设定异步数据接口的额外参数，任意设
                        name: goodInfo
                    }
                    , request: {
        ...
        var $ = layui.$, active = {
            batchdel: function () {
                var checkStatus = table.checkStatus('goodInfoTable')
                ...
                layer.confirm('确定要删除' + checkData.length + '条数据吗？', function (index) {
                    ...
                    $.ajax({
                        type: 'post',
                        data: {goods: JSON.stringify(checkData)},
                        url: "${request}/good/deleteManygoods.html",
                        success: function (data) {
                            if (data.data == "deleteSuccess") {
                                layer.msg('已删除');
                                table.reload('goodInfoTable', {
                                    url: '${request}/good/queryAllgood.html' //向后端默认传page和limit); //重载表格
                                    , request: {
                                        pageName: 'pageNum',
                                        limitName: 'pageSize'  //如不配置，默认为page=1&limit=10
                                    }
            ...
            add: function () {
                layer.open({
                    type: 2
                    , title: '添加奖惩信息'
                    , content: '${request}/good/toEditGood.html'/*表格样式goodForm*/
                    , maxmin: true
                    , area: ['820px', '730px']
                    , btn: ['确定', '取消']
                    , yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(layuiadmin-app-form-submit)', function (data) {
                            ...
                            $.ajax({
                                data: json,
                                type: 'post',
                                url: "${request}/good/insertgood.html",
            ...
            if (obj.event === 'del') {
                layer.confirm('确定删除此数据吗？', function (index) {
                    ...
                    $.ajax({
                        data: {id:data.id},
                        type: 'post',
                        url: "${request}/good/deleteOnegood.html",
                        success: function (data) {
                ...
                layer.open({
                    ...
                    title: '编辑信息'
                    ,
                    content: '${request}/good/toEditGood.html'
                    ,
                    maxmin: true
                    ,
                    area: ['820px', '730px']
                    ...
                    yes: function (index, layero) {
                        ...
                        iframeWindow.layui.form.on('submit(layuiadmin-app-form-edit)', function (data) {
                            ...
                            $.ajax({
                                data: json,
                                type: 'post',
                                url: "${request}/good/updategood.html",
                                success: function (data) {
                    ...
                    success: function (layero, index) {
                        ...
                        othis.find('input[name="detail"]').val(data.detail);
                        othis.find('input[name="reason"]').val(data.reason);
